...
# Explicit catalogue: index_name → (table, [columns], unique)
# Used by RepairEngine to issue safe CREATE INDEX IF NOT EXISTS.
# Always add new entries here when a migration creates an index.
_INDEX_CATALOGUE: dict[str, tuple[str, list[str], bool]] = {
    # 007
    "ix_pipeline_state_thread_id":     ("pipeline_state", ["thread_id"],     False),
    "ix_pipeline_state_checkpoint_id": ("pipeline_state", ["checkpoint_id"], True),
    "ix_pipeline_state_created_at":    ("pipeline_state", ["created_at"],    False),
    # 008
    "ix_pipeline_events_thread_id":    ("pipeline_events", ["thread_id"],  False),
    "ix_pipeline_events_created_at":   ("pipeline_events", ["created_at"], False),
    # 009
    "ix_migration_secrets_migration_id": ("migration_secrets",  ["migration_id"], False),
    "ix_secret_audit_log_secret_id":     ("secret_audit_log",   ["secret_id"],    False),
    "ix_secret_audit_log_migration_id":  ("secret_audit_log",   ["migration_id"], False),
    "ix_secret_audit_log_created_at":    ("secret_audit_log",   ["created_at"],   False),
    # 010
    "ix_runner_jobs_migration_id":    ("runner_jobs",      ["migration_id"], False),
    "ix_runner_jobs_status":          ("runner_jobs",      ["status"],       False),
    "ix_runner_jobs_created_at":      ("runner_jobs",      ["created_at"],   False),
    "ix_deployment_logs_job_id":      ("deployment_logs",  ["job_id"],       False),
    "ix_deployment_logs_created_at":  ("deployment_logs",  ["created_at"],   False),
    # 013
    "uq_region_availability_cache_key": ("region_availability_cache", ["service_name", "region", "provider"], True),
    "ix_region_cache_provider_region":  ("region_availability_cache", ["provider", "region"],                 False),
    "ix_region_cache_expires_at":       ("region_availability_cache", ["expires_at"],                         False),
    # 014
    "ix_ontology_source":      ("cloud_service_ontology", ["source_provider", "source_service"], False),
    "ix_ontology_pair":        ("cloud_service_ontology", ["source_provider", "source_service", "target_provider", "target_service"], True),
    "ix_audit_migration_id":   ("migration_audit", ["migration_id"],  False),
    "ix_audit_source_svc":     ("migration_audit", ["source_service"], False),
    "ix_audit_target_svc":     ("migration_audit", ["target_service"], False),
    # 016
    "ix_users_email":    ("users", ["email"],    True),
    "ix_users_username": ("users", ["username"], True),
    # 020
    "idx_audit_user_id":     ("audit_logs", ["user_id"],     False),
    "idx_audit_action":      ("audit_logs", ["action"],      False),
    "idx_audit_resource_id": ("audit_logs", ["resource_id"], False),
    "idx_audit_created_at":  ("audit_logs", ["created_at"],  False),
    # 022
    "ix_invitations_token": ("invitations", ["token"], True),
    "ix_invitations_email": ("invitations", ["email"], False),
    # 023
    "ix_migrations_user_id": ("migrations", ["user_id"], False),
}


def _index_table(index_name: str, expected: dict) -> str | None:
    """Look up table from explicit catalogue, fallback to name convention."""
    if index_name in _INDEX_CATALOGUE:
        return _INDEX_CATALOGUE[index_name][0]
    for table in expected.get("tables", []):
...
class RepairEngine:
    ...
    def repair_missing_index(self, item: DriftItem) -> str | None:
        """Re-create a missing index using the explicit catalogue."""
        idx = item.object_name
        if idx not in _INDEX_CATALOGUE:
            logger.warning("Index %s not in catalogue — cannot auto-repair", idx)
            return None
        table, cols, unique = _INDEX_CATALOGUE[idx]
        col_list = ", ".join(cols)
        u = "UNIQUE " if unique else ""
        try:
            self._exec(
                f'CREATE {u}INDEX IF NOT EXISTS "{idx}" ON {table} ({col_list})'
            )
            return f"Repaired index {idx} on {table}({col_list})"
